...
//    [self addSubview:sendButton];
    ...
    for (int i = 0; i < totalPage; i ++) {
        for (int row = 0; row < emotionManager.emotionRow; row++) {
            for (int col = 0; col < maxCol; col++) {
                ...
                if (emotionManager.emotionType != EMEmotionGif) {
                    if (index != 0 && (index - (pageSize-2))%pageSize == 0) {
        ...
        if (emotionManager.emotionType != EMEmotionGif) {
            ...
            UIButton *sendButton = [UIButton buttonWithType:UIButtonTypeCustom];
            [sendButton setTitle:NSLocalizedString(@"send", @"Send") forState:UIControlStateNormal];
            [sendButton setFrame:CGRectMake(i * CGRectGetWidth(self.frame) + (emotionManager.emotionCol - 2) * itemWidth, (emotionManager.emotionRow - 1) * itemHeight+5, itemWidth, itemHeight-10)];
            sendButton.layer.cornerRadius = 2.5;
            sendButton.titleLabel.font = fontSize_15;
            [sendButton addTarget:self action:@selector(sendAction:) forControlEvents:UIControlEventTouchUpInside];
            [sendButton setBackgroundColor:HMColor(13, 195,172)];
...
#pragma mark - pageControlChanged
-(void)pageControlChanged:(UIPageControl *)pageControl{
    
    NSInteger page = pageControl.currentPage;
    CGPoint point = _scrollview.contentOffset;
    CGFloat x = CGRectGetWidth(_scrollview.frame)*page;
    [_scrollview setContentOffset:CGPointMake(x, point.y) animated:YES];
}
@end
